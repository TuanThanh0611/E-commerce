...
exports.getGradleDependencyIdRegEx = exports.JVM_PROJECT_FILES = exports.LARGE_BUFFER = void 0;
...
function runBuilderCommand(commandAlias, getBuilder, params, options = {
    ...
    const executable = buildSystem.getExecutable(options.ignoreWrapper ?? false, options.useLegacyWrapper ?? false);
...
function hasMavenProject(cwd) {
    return (0, fileutils_1.fileExists)((0, path_1.resolve)(cwd, 'pom.xml'));
}
...
function hasGradleProject(cwd) {
    return ((0, fileutils_1.fileExists)((0, path_1.resolve)(cwd, 'build.gradle')) ||
        (0, fileutils_1.fileExists)((0, path_1.resolve)(cwd, 'settings.gradle')) ||
        (0, fileutils_1.fileExists)((0, path_1.resolve)(cwd, 'build.gradle.kts')) ||
        (0, fileutils_1.fileExists)((0, path_1.resolve)(cwd, 'settings.gradle.kts')));
}
function hasGradleSettingsFile(cwd) {
    return ((0, fileutils_1.fileExists)((0, path_1.resolve)(cwd, 'settings.gradle')) ||
        (0, fileutils_1.fileExists)((0, path_1.resolve)(cwd, 'settings.gradle.kts')));
}
...
const getGradleDependencyIdRegEx = () => /\s*(api|compileOnly|implementation|runtimeOnly|testApi|testCompileOnly|testImplementation|testRuntimeOnly)\s*(?<project>(\(?project)?\s*\(?['"]:?(?<id>[^"']+)['"]\)?\)?)/g;
    ...
    if (isGradleProject(project)) {
            ...
            do {
                ...
                if (idGroup && projectGroup) {
                    ...
                    if (/^\(?project/.test(projectGroup)) {
                        ...
                        const { groupId, artifactId } = (0, gradle_utils_1.getCoordinatesForGradleProjet)(`${project.root}/../${idGroup.replaceAll(':', '/')}`);
                        packageId = `${groupId}:${artifactId}`;
                    }
                    dependencies.push({
                        packageId,
        ...
        return {
            ...
            packageFile: hasGradleSettingsFile(project.root)
                ...
                : `build${ext}`,
            dependencies,
            modules: modules.map((module) => `${project.root}/${module.replaceAll(':', '/')}`),
        };
    }
...
async function getAdjustedProjectAndModuleRoot(options, isMavenProject) {
    const projectGraph = process.env['NX_INTERACTIVE'] === 'true'
        ? (0, devkit_1.readCachedProjectGraph)()
        : await (0, devkit_1.createProjectGraphAsync)();
