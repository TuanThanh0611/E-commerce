...
async function generateBootProject(tree, options) {
    const downloadUrl = (0, boot_utils_1.buildBootDownloadUrl)(options);
    devkit_1.logger.info(`⬇️ Downloading Spring Boot project zip from : ${downloadUrl}...`);
    const response = await (0, node_fetch_1.default)(downloadUrl, (0, common_jvm_1.getCommonHttpHeaders)(index_1.NX_SPRING_BOOT_PKG, downloadUrl, options.proxyUrl));
    devkit_1.logger.info(`📦 Extracting Spring Boot project zip to '${(0, devkit_1.joinPathFragments)(devkit_1.workspaceRoot, options.projectRoot)}'...`);
    if (response.ok) {
        await (0, common_jvm_1.extractFromZipStream)(response.body, (entryPath, entryContent) => {
            const execPermission = entryPath.endsWith('mvnw') || entryPath.endsWith('gradlew')
                ? '755'
                : undefined;
            if ((0, common_jvm_1.getMavenWrapperFiles)().includes(entryPath) ||
                (0, common_jvm_1.getGradleWrapperFiles)().includes(entryPath)) {
...
function getBootApplicationOnlyFiles(options) {
    ...
    const ext = options.language === 'kotlin'
        ? '.kt'
